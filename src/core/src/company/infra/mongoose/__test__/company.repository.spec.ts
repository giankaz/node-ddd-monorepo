import { CompanyMongoRepository } from '../';
import {
  CommonStatus,
  FilterOperators,
  FilterParams,
...
describe('mongoose repository tests', () => {
  ...
  >;
  let randomEntity: Company;

  beforeEach(() => {
    repository = new CompanyMongoRepository();
    randomEntity = RandomCompanyFactory.createOne();
  });
  ...
  it('should run the method search using filters', async () => {
    ...
    const filter: FilterParams<CompanyRepositoryInterface.CompanyFields>[] = [
      {
        column: 'name',
    ...
    const filter2: FilterParams<CompanyRepositoryInterface.CompanyFields>[] = [
      {
        column: 'name',
        operator: FilterOperators.CONTAINS,
        type: 'string',
    ...
    const filter3: FilterParams<CompanyRepositoryInterface.CompanyFields>[] = [
      {
        column: 'id',
