import { ExampleMongoRepository } from '../';
import {
  CommonStatus,
  FilterOperators,
  FilterParams,
...
import { Example, ExampleModel, RandomExampleFactory } from '../../../domain';

describe('mongoose repository tests', () => {
  let repository: RepositoryInterface<ExampleModel, Example>;
  let randomEntity: Example;

  beforeEach(() => {
    repository = new ExampleMongoRepository();
    randomEntity = RandomExampleFactory.createOne();
  });
  ...
  it('should find by field', async () => {
    ...
    const result = await repository.findByField('value', randomEntity.value);
  ...
  it('should run the method search using filters', async () => {
    const entities = RandomExampleFactory.createMultiple(10);
  ...
  it('should run the method update', async () => {
    ...
    expect(foundAfterUpdate.value).toStrictEqual(NEW_VALUE);
  });
