import { LeadMongoRepository } from '../';
import {
  CommonStatus,
  FilterOperators,
  FilterParams,
...
describe('mongoose repository tests', () => {
  ...
  >;
  let randomEntity: Lead;

  beforeEach(() => {
    repository = new LeadMongoRepository();
    randomEntity = RandomLeadFactory.createOne();
  });
  ...
  it('should run the method search using filters', async () => {
    const entities = RandomLeadFactory.createMultiple(10);
    ...
    const filter: FilterParams<LeadRepositoryInterface.LeadFields>[] = [
      {
        column: 'name',
    ...
    const filter2: FilterParams<LeadRepositoryInterface.LeadFields>[] = [
      {
        column: 'name',
        operator: FilterOperators.CONTAINS,
        type: 'string',
    ...
    const filter3: FilterParams<LeadRepositoryInterface.LeadFields>[] = [
      {
        column: 'id',
