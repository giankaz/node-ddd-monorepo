...
import { ListActivitysUseCase } from '../list-activitys.use-case';
...
describe('List Activitys UseCase Test', () => {
  let useCase: ListActivitysUseCase.UseCase;
  let repository: ActivityRepositoryInterface.Repository;

  beforeEach(() => {
    repository = new ActivityInMemoryRepository();
    useCase = new ListActivitysUseCase.UseCase(repository);
  });
  ...
  it('should work with default search', async () => {
    const items = RandomActivityFactory.createMultiple(2);

    const FAKE_ID = uuid();

    const sameIdItems = [
      RandomActivityFactory.createOne({
        id: FAKE_ID,
      }),
      RandomActivityFactory.createOne({
        id: FAKE_ID,
  ...
  it('should work the page and per_page methods', async () => {
    const FAKE_ID = uuid();
    const items = RandomActivityFactory.createMultiple(21, {
      id: FAKE_ID,
    });
  ...
  it('should work the search method', async () => {
    ...
    const items = RandomActivityFactory.createMultiple(5, {
      name: FAKE_ID,
    });
  ...
  it('should work the filter method', async () => {
    ...
    const items = [
      ...
      ...RandomActivityFactory.createMultiple(4, {
