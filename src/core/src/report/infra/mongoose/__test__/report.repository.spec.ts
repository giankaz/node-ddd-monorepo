import { ReportMongoRepository } from '../';
import {
  CommonStatus,
  FilterOperators,
  FilterParams,
...
describe('mongoose repository tests', () => {
  ...
  >;
  let randomEntity: Report;

  beforeEach(() => {
    repository = new ReportMongoRepository();
    randomEntity = RandomReportFactory.createOne();
  });
  ...
  it('should run the method search using filters', async () => {
    const entities = RandomReportFactory.createMultiple(10);
    ...
    const filter: FilterParams<ReportRepositoryInterface.ReportFields>[] = [
      {
        column: 'name',
    ...
    const filter2: FilterParams<ReportRepositoryInterface.ReportFields>[] = [
      {
        column: 'name',
        operator: FilterOperators.CONTAINS,
        type: 'string',
    ...
    const filter3: FilterParams<ReportRepositoryInterface.ReportFields>[] = [
      {
        column: 'id',
