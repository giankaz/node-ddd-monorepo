import { ImportationMongoRepository } from '../';
import {
  CommonStatus,
  FilterOperators,
  FilterParams,
...
describe('mongoose repository tests', () => {
  ...
  >;
  let randomEntity: Importation;

  beforeEach(() => {
    repository = new ImportationMongoRepository();
    randomEntity = RandomImportationFactory.createOne();
  });
  ...
  it('should run the method search using filters', async () => {
    const entities = RandomImportationFactory.createMultiple(10);
    ...
    const filter: FilterParams<ImportationRepositoryInterface.ImportationFields>[] =
      [
        {
          column: 'name',
    ...
    const filter2: FilterParams<ImportationRepositoryInterface.ImportationFields>[] =
      [
        {
          column: 'name',
          operator: FilterOperators.CONTAINS,
    ...
    const filter3: FilterParams<ImportationRepositoryInterface.ImportationFields>[] =
      [
        {
          column: 'id',
