...
describe('List Leads UseCase Test', () => {
  let useCase: ListLeadsUseCase.UseCase;
  let repository: LeadRepositoryInterface.Repository;

  beforeEach(() => {
    repository = new LeadInMemoryRepository();
    useCase = new ListLeadsUseCase.UseCase(repository);
  });
  ...
  it('should work with default search', async () => {
    const items = RandomLeadFactory.createMultiple(2);

    const FAKE_ID = uuid();

    const sameIdItems = [
      RandomLeadFactory.createOne({
        id: FAKE_ID,
      }),
      RandomLeadFactory.createOne({
        id: FAKE_ID,
  ...
  it('should work the page and per_page methods', async () => {
    const FAKE_ID = uuid();
    const items = RandomLeadFactory.createMultiple(21, {
      id: FAKE_ID,
    });
  ...
  it('should work the search method', async () => {
    ...
    const items = RandomLeadFactory.createMultiple(5, {
      name: FAKE_ID,
    });
  ...
  it('should work the filter method', async () => {
    ...
    const items = [
      ...
      ...RandomLeadFactory.createMultiple(4, {
